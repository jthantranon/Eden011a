...
MEUI.NewSheet = function (props) {
	...
	// template properties
	this.CSSFILE = 'newglass.css';
			
	this.wrapperStart = '<div class="newsheetWrapper" id="' + this.id + '">';
	this.wrapperEnd = '</div>';
	...
	this.init = function() {
		...
		this.$domctx.jqoteapp('#newglassTmpl');
		this.$domctx = this.$domctx.children('.newsheetWrapper');
		this.$domctx.unwrap();
// 	
// 		
		// var $ctx = this.$domctx;
...
// 		
		this.enableDrag();
		this.enableResize($ctx.children('.sheetContent'));
		// this.enableFade();
// 							
		// // position fix if draggble/resize is added
		$ctx.css({'position':'fixed'})	
		
		this.$domctx.children('.sheetRollup').on('click', function(e){
			if(this.isRolledUp){
				$ctx.children('.sheetContent').show();
				this.isRolledUp = false;
			} else {
				$ctx.children('.sheetContent').hide();
...
MEUI.NewSheet.prototype = {
		
	constructor: MEUI.NewSheet,
	...
	enableDrag: function(handle) {
		...
		if(typeof handle === 'undefined') {	
			self.$domctx.draggable({snap:"window, .newsheetWrapper",
									containment: "window",
									snapTolerance: 20 }); }
