...
MEUI.TestGlass = function (props) {
	...
	var init = function() {
		...
		// content
		self.$domctx.children('.glassContent').append(
			'<input type="text" class="testGlassName" name="testGlassInputOne' + this.id + '">' +
			'<input type="text" class="testGlassWishArgs" name="testGlassInputTwo' + this.id + '"><br>' +
			'<button id="testGlassSubmit' + this.id + '">Submit</button>'+
			'<h4>Response:</h4>'+
			'<div class="testGlassResponse" id="testGlassResponse' + this.id + '"></div');
	...
	this.bindings = function() {
		...
		$ctx.find('button').on('click',function(e){
			var name = $ctx.find('.testGlassName').val();
			var wishargs = $ctx.find('.testGlassWishArgs').val();
						
			MEUI.Wish(name, wishargs, function(d){
				$ctx.find('.testGlassResponse').empty();
				$ctx.find('.testGlassResponse').append(JSON.stringify(d));				
			});
...
MEUI.TestGlass.prototype = {
		
	constructor: MEUI.TestGlass,
